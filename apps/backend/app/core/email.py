...
class EmailService:
    ...
    def send_email(email: Email):
        ...
        sender_info = {
            "name": os.getenv("EMAIL_SENDER_NAME", email.sender_name),
            "email": os.getenv("EMAIL_SENDER_EMAIL", email.sender_email),
        }
        ...
        # If backend is Brevo, allow sending even in development
        if backend == "brevo":
            sender_info = {
                "name": os.getenv("EMAIL_SENDER_NAME", email.sender_name),
                "email": os.getenv("EMAIL_SENDER_EMAIL", email.sender_email),
            ...
            for recipient in email.recipients:
                ...
                try:
                    ...
                    EmailService.logger.exception(f"Brevo send failed: {e}")
                    raise
                anonymized_recipient = EmailService._anonymize_email(recipient)
                EmailService.logger.info(f"Email sent successfully to {anonymized_recipient} (backend=brevo, env={env})")
            return

        # Default: LOG in development if not using Brevo; send in production
        if env == "development":
            for recipient in email.recipients:
                EmailService.logger.info(f"[DEV] Would send email via backend={backend} to: {recipient}")
            return
        else:
            sender_info = {
                "name": os.getenv("EMAIL_SENDER_NAME", email.sender_name),
